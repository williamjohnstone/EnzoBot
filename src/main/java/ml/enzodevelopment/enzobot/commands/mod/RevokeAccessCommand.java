...
public class RevokeAccessCommand implements Command {
        ...
        if (requester.hasPermission(requiredPermissions) && requester.hasPermission(event.getMessage().getMentionedChannels().get(0), requiredPermissions)) {
            if (event.getMessage().getMentionedMembers().size() == 1) {
                if (event.getMember().canInteract(event.getMessage().getMentionedMembers().get(0))) {
                    removeAccess(event, event.getMessage().getMentionedMembers().get(0), null, event.getMessage().getMentionedChannels().get(0));
                }
            }
        } else if (event.getMessage().getMentionedRoles().size() == 1) {
            if (event.getMember().canInteract(event.getMessage().getMentionedRoles().get(0))) {
                removeAccess(event, null, event.getMessage().getMentionedRoles().get(0), event.getMessage().getMentionedChannels().get(0));
            }
        }
    }
    ...
    private void removeAccess(GuildMessageReceivedEvent event, Member m, Role r, TextChannel channel) {
        if (m == null && r != null) {
            removeAccess(event, r, channel);
        } else if (r == null && m != null) {
          removeAccess(event, m, channel);
        }
    }

    private void removeAccess(GuildMessageReceivedEvent event, Member m, TextChannel channel) {
        if (event.getGuild().getSelfMember().canInteract(m)) {
            channel.createPermissionOverride(m).setDeny(Permission.VIEW_CHANNEL).queue();
        }
    }

    private void removeAccess(GuildMessageReceivedEvent event, Role r, TextChannel channel) {
        if (event.getGuild().getSelfMember().canInteract(r)) {
            channel.createPermissionOverride(r).setDeny(Permission.VIEW_CHANNEL).queue();
        }
    }
}
